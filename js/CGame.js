function CGame(oData){
    ...
    this.generateFinalSymbols = function(){
        ...
        for(var k=0;k<_iLastLineActive;k++){
            ...
            while(WILD_SYMBOLS.includes(iValue) && iStartIndex<NUM_REELS){
                iNumEqualSymbol++;
            ...
            for(var t=iStartIndex;t<aCombos.length;t++){
                if(_aFinalSymbolCombo[aCombos[t].row][aCombos[t].col] === iValue || 
                                             WILD_SYMBOLS.includes(_aFinalSymbolCombo[aCombos[t].row][aCombos[t].col])){
                    iNumEqualSymbol++;
    ...
    this._endReelAnimation = function(){
        ...
        _oInterface.disableBetBut(_freeSpin);
        ...
        if(_freeSpin) {
            _oInterface.refreshFreeSpinText(_freeSpin);
            _oInterface.disableFreeSpinGuiButtons();
        } else {
    ...
    this.onSpin = function(){
        ...
        var requestOptions = {
            method: 'POST',
            headers: { "content-type": "application/json", "uuid": sessionId },
            body: JSON.stringify({
                bet: _iCurBet,
                totalBet: _iTotBet,
        ...
        fetch(apiUrl+`/api/slots/spin`, requestOptions)
            .then(res => res.json())
            .then(it => {
                var d = it.playedLines;
                _aFinalSymbolCombo = new Array();
                _aFinalSymbolCombo[0] = new Array();
                _aFinalSymbolCombo[0][0] = d[0].s1.number;
                _aFinalSymbolCombo[0][1] = d[0].s2.number;
                _aFinalSymbolCombo[0][2] = d[0].s3.number;
                _aFinalSymbolCombo[0][3] = d[0].s4.number;
                _aFinalSymbolCombo[0][4] = d[0].s5.number;

                _aFinalSymbolCombo[1] = new Array();
                _aFinalSymbolCombo[1][0] = d[1].s1.number;
                _aFinalSymbolCombo[1][1] = d[1].s2.number;
                _aFinalSymbolCombo[1][2] = d[1].s3.number;
                _aFinalSymbolCombo[1][3] = d[1].s4.number;
                _aFinalSymbolCombo[1][4] = d[1].s5.number;

                _aFinalSymbolCombo[2] = new Array();
                _aFinalSymbolCombo[2][0] = d[2].s1.number;
                _aFinalSymbolCombo[2][1] = d[2].s2.number;
                _aFinalSymbolCombo[2][2] = d[2].s3.number;
                _aFinalSymbolCombo[2][3] = d[2].s4.number;
                _aFinalSymbolCombo[2][4] = d[2].s5.number;
                
                _oInterface.hideAllLines();
                _oInterface.disableGuiButtons();

                it.wonLines.forEach(w => {
                    var _aCellList = new Array();
                    w.xys.slice(0, w.count).forEach(xy => {
                        _aCellList.push({row:xy.x, col:xy.y, value:xy.symbol.number });
                    });
                    _aWinningLine.push({
                        line: w.line, 
                        amount: w.count, 
                        num_win: w.count,
                        value: w.xys[0].symbol.number,
                        list: _aCellList
                    });
                });

                if(it.wonLines.length>0) {
                    _iTotWin = it.wonAmount;
                }

                _freeSpin = it.freeSpin;
